...
# Classes du dataset (basées sur l'ordre alphabétique des dossiers)
...
# Transformation pour PyTorch (conforme à prep.py)
...
def predict():
    ...
    try:
        img = Image.open(io.BytesIO(file.read())).convert('L')  # Convertir en niveaux de gris
        
        if model_type == 'pytorch':
            model = load_pytorch_model()
            img_tensor = pytorch_transform(img).unsqueeze(0)
            with torch.no_grad():
                output = model(img_tensor)  # Sorties brutes (logits)
                probs = torch.softmax(output, dim=1)[0] * 100  # Convertir en pourcentages
                pred = output.max(1)[1].item()  # Indice de la classe prédite
            ...
            output = model.predict(img_array, verbose=0)  # Sorties brutes (probabilités via softmax)
            pred = output.argmax()
            prediction = CLASS_NAMES[pred]
            probs = output[0] * 100  # Convertir en pourcentages
